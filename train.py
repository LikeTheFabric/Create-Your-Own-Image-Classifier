...
parser.add_argument('data_dir', action="store", defualt="./flowers/")
parser.add_argument('--save_dir', action="store", defualt='./checkpoint.pth')
parser.add_argument('--arch', action="store", default="vgg16")
parser.add_argument('--learning_rate', action="store", type=float,defualt=0.001)
    ...
    for epoch in range(epochs):
        for inputs, labels in trainloader:
            ...
            if steps % print_every == 0:
                ...
                model.eval()
                with torch.no_grad():
                    for intpus, labels in validloader: 
                        inputs, labels = inputs.to(device), labelss.to(device)
